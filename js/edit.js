        ...
        // 트랜스크립션 렌더링 (탭 기반 - 원본/번역 구분) (최적화)
        function renderTranscriptions() {
            const list = DOMCache.transcriptionList;
            if (!list) return;
            
            // 원본/번역 탭에 따라 표시할 언어 결정
            let displayLangCode = currentLang;
            let displayLangInfo = availableLanguages.find(lang => lang.code === currentLang) || availableLanguages[0];
            
            if (currentTab === 'original') {
                // 원본 탭: 원본 언어 표시
                if (originalLang && originalLang !== 'auto') {
                    displayLangCode = originalLang;
                } else {
                    // auto인 경우 첫 번째 원본 언어 찾기
                    const firstOriginalLang = availableLanguages.find(lang => lang.isOriginal);
                    if (firstOriginalLang) {
                        displayLangCode = firstOriginalLang.code;
                    }
                }
                displayLangInfo = availableLanguages.find(lang => lang.code === displayLangCode) || availableLanguages[0];
            } else {
                // 번역 탭: 번역된 언어 표시
                const firstTargetLang = availableLanguages.find(lang => !lang.isOriginal);
                if (firstTargetLang) {
                    displayLangCode = firstTargetLang.code;
                    displayLangInfo = firstTargetLang;
                }
            }
            ...
            list.innerHTML = transcriptions.map(segment => {
                ...
                // 표시할 언어의 텍스트 가져오기
                const text = segment[displayLangCode] || segment[getLanguageFieldName(displayLangCode)] || '';
                const placeholder = displayLangInfo.isOriginal ? `${displayLangInfo.name} 자막을 입력하세요` : `${displayLangInfo.name} subtitle`;
                ...
                // 언어 정보 가져오기
                const langInfo = getLanguageInfo(displayLangCode);
                        ...
                        <div class="text-content">
                            ...
                            <div class="translation-text">
                                <div class="output-label">
                                    <span>${displayLangInfo.name || langInfo.name}</span>
                                </div>
                                <textarea class="text-input" data-lang="${displayLangCode}" data-segment-id="${segment.id}" placeholder="${placeholder}">${outputText}</textarea>
                            </div>
                        </div>
                    </div>
                `;
        ...
        function updateVideoMode() {
            ...
            // 원본/번역 모드에 따라 자막 표시 및 렌더링
            if (currentTab === 'translation') {
                showSubtitles = true;
                // 번역 탭: 번역된 자막 표시
                // 첫 번째 대상 언어를 기본으로 선택
                if (targetLanguages.length > 0 && availableLanguages.length > 0) {
                    const firstTargetLang = availableLanguages.find(lang => !lang.isOriginal);
                    if (firstTargetLang) {
                        currentLang = firstTargetLang.code;
                    }
                }
                // 자막 버튼 표시
                if (ccBtn) {
                ...
                if (originalLang && originalLang !== 'auto') {
                    ...
                    // auto인 경우 첫 번째 사용 가능한 언어 사용
                    const firstOriginalLang = availableLanguages.find(lang => lang.isOriginal);
                    if (firstOriginalLang) {
                        currentLang = firstOriginalLang.code;
                    }
                }
                // 자막 버튼 숨김
                if (ccBtn) {
            ...
            if (currentSegment) {
                ...
                if (currentTab === 'original') {
                    ...
                    if (originalLang && originalLang !== 'auto') {
                        ...
                        const firstOriginalLang = availableLanguages.find(lang => lang.isOriginal);
                        if (firstOriginalLang) {
                            langCode = firstOriginalLang.code;
                        }
                    }
                } else {
                    // 번역 탭: 번역된 자막 표시
